...
const ROWS = [
  ...
  },
  {
    ...
    endpoint:
      "/discover/movie?with_genres=28&sort_by=popularity.desc&vote_count.gte=100",
    mediaType: "movie",
  },
  {
    id: "comedy",
    title: "Comedies",
    endpoint:
      "/discover/movie?with_genres=35&sort_by=popularity.desc&vote_count.gte=100",
    mediaType: "movie",
  },
  {
    id: "horror",
    title: "Horror",
    endpoint:
      "/discover/movie?with_genres=27&sort_by=popularity.desc&vote_count.gte=50",
    mediaType: "movie",
  },
  {
    id: "scifi",
    title: "Sci-Fi",
    endpoint:
      "/discover/movie?with_genres=878&sort_by=popularity.desc&vote_count.gte=100",
    mediaType: "movie",
  },
  {
    id: "romance",
    ...
    endpoint:
      "/discover/movie?with_genres=10749&sort_by=popularity.desc&vote_count.gte=50",
    mediaType: "movie",
  },
  {
    id: "thriller",
    title: "Thrillers",
    endpoint:
      "/discover/movie?with_genres=53&sort_by=popularity.desc&vote_count.gte=100",
    mediaType: "movie",
  },
  {
    id: "docs",
    title: "Documentaries",
    endpoint:
      "/discover/movie?with_genres=99&sort_by=popularity.desc&vote_count.gte=20",
    mediaType: "movie",
  },
  {
    id: "animation",
  ...
  },
  {
    ...
    endpoint:
      "/discover/tv?with_genres=80&sort_by=popularity.desc&vote_count.gte=50",
    mediaType: "tv",
  },
];
...
async function tmdb(ep, extra = {}) {
  ...
  if (IS_LOCAL) {
    ...
    Object.entries(extra).forEach(
      ([k, v]) => (url += `&${k}=${encodeURIComponent(v)}`),
    );
  } else {
...
async function loadGenres() {
  try {
    ...
    [...(m.genres || []), ...(t.genres || [])].forEach(
      (g) => (GENRE_MAP[g.id] = g.name),
    );
    buildFilterMenu();
  } catch (e) {}
}
...
function buildFilterMenu() {
  ...
  const genres = Object.entries(GENRE_MAP).sort((a, b) =>
...
async function loadNextPage() {
  ...
  try {
    ...
    if (results.length === 0) {
      document.getElementById("infinite-scroll-sentinel").style.display =
        "none";
    }
  } catch (e) {
...
function makeCard(item, badgeType, idx) {
  ...
  panel.querySelector('[data-do="play"]').onclick = (e) => {
    e.stopPropagation();
    playContent(item);
  };
...
function renderHero() {
  ...
  document.getElementById("hero-type-text").textContent =
    heroItem.type === "tv" ? "S E R I E S" : "F I L M";
...
async function openDetail(item, updateUrl = true) {
  ...
  document.getElementById("detail-hero").style.backgroundImage = item.backdrop
    ? `url(${item.backdrop})`
    : "none";
  ...
  b.className = "badge";
  b.textContent = item.type === "tv" ? "Series" : "Movie";
  meta.appendChild(b);
  ...
  document.getElementById("detail-genre-line").innerHTML = genreNames(
    item.genreIds,
  ).length
  ...
  if (item.type === "tv" && pr && pr.season && pr.episode) {
    pb.querySelector("span").textContent =
      `Resume S${pr.season}:E${pr.episode}`;
    pb.onclick = () => playContent(item, pr.season, pr.episode);
  } else if (pr && pr.currentTime > 0 && (pr.progress || 0) < 95) {
    pb.querySelector("span").textContent = "Resume";
  ...
  try {
    ...
    if (cast.length)
      document.getElementById("detail-cast-line").innerHTML =
        `<span>Cast:</span> ${cast.map((c) => c.name).join(", ")}`;

    const abt = document.getElementById("about-details");
    const rows = [];
    if (cast.length)
      rows.push(
        `<div class="about-row"><strong>Cast:</strong> ${cast.map((c) => c.name).join(", ")}</div>`,
      );
    if (det.genres)
      rows.push(
        `<div class="about-row"><strong>Genres:</strong> ${det.genres.map((g) => g.name).join(", ")}</div>`,
      );
    if (det.status)
      rows.push(
        `<div class="about-row"><strong>Status:</strong> ${det.status}</div>`,
      );
    if (det.runtime)
      rows.push(
        `<div class="about-row"><strong>Runtime:</strong> ${det.runtime} min</div>`,
      );
    if (det.vote_average)
      rows.push(
    ...
    if (item.type === "tv" && det.seasons) {
      ...
      if (seasons.length) {
        meta.querySelector(".badge").textContent =
          `${seasons.length} Season${seasons.length > 1 ? "s" : ""}`;
        const pick = document.getElementById("season-picker");
        pick.innerHTML = seasons
          .map(
            (s) =>
    ...
    const sims = (sim.results || [])
      .slice(0, 9)
      .map((r) => norm(r, item.type))
      .filter(Boolean);
...
async function fetchEps(tvId, sNum) {
  ...
  try {
    ...
    list.innerHTML =
      '<div class="empty-state"><svg width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#ff4444" stroke-width="1"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg><h3 style="color:#ff4444">Failed to load</h3><p>Please try again later.</p><button class="btn-hero btn-gray" style="margin-top: 20px;" onclick="closeDetail()">Go Back</button></div>';
  }
}
...
async function applyFilter(genreId, genreName) {
  ...
  document.getElementById("search-heading").innerHTML =
    `Movies & Shows: <span>${escapeHtml(genreName)}</span>`;
  const grid = document.getElementById("search-grid");
  grid.innerHTML =
    '<div class="loading-spinner-wrap active" style="grid-column: 1 / -1;"><div class="spinner"></div></div>';
  ...
  try {
    ...
    if (!items.length) {
      grid.innerHTML =
        '<div class="empty-state" style="grid-column: 1 / -1;"><svg width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#555" stroke-width="1"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" /></svg><h3>No matches found</h3><p>Try selecting a different genre.</p><button class="btn-hero btn-white" style="margin-top: 20px;" onclick="navTo(\'home\')">Browse Home</button></div>';
      return;
    }
    grid.innerHTML = "";
    items.forEach((i) => grid.appendChild(makeCard(i)));
    ...
    grid.innerHTML =
      '<div class="empty-state" style="grid-column: 1 / -1;"><svg width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#ff4444" stroke-width="1"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg><h3 style="color:#ff4444">Something went wrong</h3><p>Please try again later.</p><button class="btn-hero btn-white" style="margin-top: 20px;" onclick="navTo(\'home\')">Browse Home</button></div>';
  }
}
...
async function doSearch(q) {
  ...
  document.getElementById("search-heading").innerHTML =
    `Search results for "<span>${escapeHtml(q)}</span>"`;
  const grid = document.getElementById("search-grid");
  grid.innerHTML =
    '<div class="loading-spinner-wrap active" style="grid-column: 1 / -1;"><div class="spinner"></div></div>';
...
async function fetchSuggestions(q) {
  ...
  try {
    ...
    document.getElementById("suggest-see-all").onclick = () => {
      box.classList.remove("active");
      box.innerHTML = "";
      doSearch(q);
    };
...
function getMyList() {
  return JSON.parse(localStorage.getItem("vk_mylist") || "[]");
}
...
function showMyList() {
  const ls = getMyList(),
    g = document.getElementById("mylist-grid"),
    em = document.getElementById("mylist-empty");
  g.innerHTML = "";
  if (!ls.length) {
...
function buildContinueRow() {
  ...
  main.insertBefore(
    makeRow(
      { id: "continue", title: "Continue Watching for You", mediaType: "all" },
      h,
    ),
    main.firstChild,
  );
...
function wireListeners() {
  ...
  document.getElementById("search-btn").onclick = () => {
    ...
    if (sw.classList.contains("open")) setTimeout(() => si.focus(), 50);
    else {
      si.value = "";
      sw.classList.remove("has-text");
      doSearch("");
      hideSuggestions();
    }
  };
  ...
  document.addEventListener("click", (e) => {
    if (
      filterDrop.classList.contains("open") &&
      !filterDrop.contains(e.target) &&
      !filterBtn.contains(e.target) &&
      (!mobileFilterTrigger || !mobileFilterTrigger.contains(e.target))
    ) {
...
function switchSyncTab(tab) {
  ...
  document
    .getElementById("sync-export-panel")
    .classList.toggle("hidden", tab !== "export");
  document
    .getElementById("sync-import-panel")
...
async function importSyncCode() {
  ...
  try {
    ...
    const msg =
      e.message === "Code not found or expired"
...
(function wireSyncModal() {
  ...
  document.getElementById("sync-overlay").onclick = (e) => {
    if (e.target === e.currentTarget) closeSyncModal();
  };
  document
    .querySelectorAll(".sync-tab")
    .forEach((t) => (t.onclick = () => switchSyncTab(t.dataset.tab)));
